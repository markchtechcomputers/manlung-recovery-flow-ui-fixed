...
// Block common SQL-injection payloads without treating ordinary case notes
// or chat punctuation as malicious SQL. Database access must still use
// parameterized/query-builder operations.
...
// Prompt-injection markers are rejected in fields that may later be passed
// to automation/AI. This is defense-in-depth; never treat user text as a
...
function enforceLoginLockout(req, res) {
  ...
  if (record.failures >= LOGIN_MAX_FAILURES) {
    res.status(429).json({
      success: false,
      error: 'Account temporarily blocked after 3 failed login attempts. Please wait 15 minutes before trying again.',
      code: 'LOGIN_LOCKED',
    });
    return false;
  }
...
function inspectString(value, keyPath) {
  ...
  for (const pattern of DANGEROUS_HTML) {
    if (pattern.test(value)) return 'Potentially executable HTML or script content detected.';
  }

  for (const pattern of SQL_INJECTION_PATTERNS) {
    if (pattern.test(value)) return 'Potentially malicious SQL input detected.';
  }

  if (PROMPT_LIKE_KEYS.has(key)) {
    for (const pattern of PROMPT_INJECTION_PATTERNS) {
      if (pattern.test(value)) return 'Potential prompt-injection content detected.';
    }
  }
...
function inspectValue(value, keyPath = '') {
  ...
  if (Array.isArray(value)) {
    for (let i = 0; i < value.length; i += 1) {
      const error = inspectValue(value[i], `${keyPath}[${i}]`);
...
function enforceOriginForCookieAuth(req, res) {
  ...
  const valid = (origin && allowed.has(origin)) || (referer && [...allowed].some(base => referer.startsWith(`${base}/`)));
...
function inputSecurity(req, res, next) {
  try {
    ...
    if (!enforceOriginForCookieAuth(req, res)) return;
