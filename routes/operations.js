...
function validReopenStatus(value) {
  const next = cleanText(value, 80) || 'Under Investigation';
  return ACTIVE.includes(next) && next !== 'Pending Review' ? next : null;
}
...
router.patch('/cases/:caseId/lifecycle', adminAuth, async (req,res) => {
  try {
    const row = await Case.findByCaseId(cleanCaseId(req.params.caseId));
...
router.patch('/cases/:caseId/priority', adminAuth, async (req,res) => {
  try {
    const caseId = cleanCaseId(req.params.caseId);
    const row = await Case.findByCaseId(caseId);
    if (!row) return res.status(404).json({success:false,error:'Case not found.'});
    if (!adminCanMutate(row, req.user)) return res.status(403).json({success:false,error:'Not authorized.'});
    const priority = cleanText(req.body.priority, 20).toLowerCase();
    if (!PRIORITIES.includes(priority)) return res.status(400).json({success:false,error:'Invalid priority.'});
    const updated = await Case.update(row.case_id,{priority,last_status_changed_at:new Date().toISOString()});
    await event(row.case_id,req.user,'priority_changed',`Case priority changed from "${row.priority || 'normal'}" to "${priority}".`,{from:row.priority || 'normal',to:priority});
    res.json({success:true,case:updated});
  } catch(e) { res.status(500).json({success:false,error:'Could not change case priority.'}); }
});

router.patch('/cases/:caseId/sla', ownerAuth, async (req,res) => {
  try {
    const caseId = cleanCaseId(req.params.caseId);
    const row = await Case.findByCaseId(caseId);
    if (!row) return res.status(404).json({success:false,error:'Case not found.'});
    const dueAt = validIsoDate(req.body.slaDueAt);
    if (req.body.slaDueAt && !dueAt) return res.status(400).json({success:false,error:'Invalid SLA due date.'});
    const updated = await Case.update(row.case_id,{sla_due_at:dueAt});
    await event(row.case_id,req.user,'sla_changed',`Case SLA ${dueAt ? 'set to ' + dueAt : 'cleared'}.`,{sla_due_at:dueAt});
    res.json({success:true,case:updated});
  } catch(e) { res.status(500).json({success:false,error:'Could not update case SLA.'}); }
});
...
router.post('/cases/:caseId/reopen', adminAuth, async (req,res) => {
  try {
    ...
    const next = validReopenStatus(req.body.status);
    if (!next) return res.status(400).json({success:false,error:'Invalid reopen status.'});
    const now = new Date().toISOString();
    const updated = await Case.update(row.case_id,{status:next,reopened_at:now,closed_at:null,last_status_changed_at:now});
    await event(row.case_id,req.user,'case_reopened',`Case reopened as "${next}".`);
...
router.post('/cases/:caseId/close', adminAuth, async (req,res) => {
  // Explicit close endpoint keeps closure auditable without mutating the request body.
  try {
