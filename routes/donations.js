...
function donationTargetKes() {
  ...
  return Number.isInteger(configured) && configured > 0 ? configured : DEFAULT_DONATION_TARGET_KES;
}
...
router.post('/initialize', [
  ...
  try {
    ...
    let { error: insertError } = await supabase.from(TABLE).insert({
      reference,
      donor_email: email,
    ...
    // Backward compatibility for deployments where migration 009 has not
    // been applied yet. Payment must not fail merely because donor_name is
    // not present in an older recovery_donations table.
    if (insertError && donorName && /donor_name|column .* does not exist|PGRST204/i.test(String(insertError.message || insertError.details || ''))) {
      ({ error: insertError } = await supabase.from(TABLE).insert({
        reference,
        donor_email: email,
        amount_kes: amountKes,
        status: 'pending',
...
router.get('/total', async (_req, res) => {
  try {
    ...
    const targetKes = donationTargetKes();
    const percentage = Math.min(100, Math.max(0, (totalKes / targetKes) * 100));
    res.set('Cache-Control', 'no-store, no-cache, must-revalidate');
    res.json({ success: true, totalKes, donationCount: rows.length, targetKes, percentage: Number(percentage.toFixed(2)) });
  } catch (error) {
...
router.get('/admin', adminAuth, async (_req, res) => {
  try {
    let { data: confirmedRows, error: confirmedError } = await supabase
      .from(TABLE)
    ...
    // Older databases may not have donor_name yet. Keep the dashboard usable
    // while the migration is being applied.
    if (confirmedError && /donor_name|column .* does not exist|PGRST204/i.test(String(confirmedError.message || confirmedError.details || ''))) {
      ({ data: confirmedRows, error: confirmedError } = await supabase
        .from(TABLE)
