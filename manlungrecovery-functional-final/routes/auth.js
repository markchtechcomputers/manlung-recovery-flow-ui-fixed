...
const signToken = (user) =>
  jwt.sign(
    ...
    {
      // Client bearer tokens are intentionally shorter-lived because they are
      // held by the browser client. Admin/Owner sessions retain the existing
      // configurable lifetime and are additionally protected by HttpOnly cookies
      // and MFA where enabled.
      expiresIn: ['admin', 'owner'].includes(user.role)
        ? (process.env.JWT_EXPIRE || '7d')
        : (process.env.CLIENT_JWT_EXPIRE || '12h'),
    }
  );
...
// ============================================================
// CLIENT GOOGLE CLOUD OAUTH
// Google Identity Services -> Google ID token -> Manlung JWT
// Supabase is NOT used for Google authentication.
// ============================================================
