...
// ============================================================
// GITHUB OAUTH CLIENT SIGN-UP / SIGN-IN
// Uses GitHub directly so the client flow does not depend on a
// Supabase provider being enabled in the browser.
// ============================================================
function githubCookieOptions() {
  return [
    'Path=/',
    'HttpOnly',
    'Secure',
...
router.get('/github/start', async (req, res) => {
  try {
    ...
    const appUrl = String(process.env.PUBLIC_APP_URL || `https://manlungrecovery.manlungshop.co.ke`).replace(/\/$/, '');
    const redirectUri = String(process.env.GITHUB_REDIRECT_URI || `${appUrl}/api/auth/github/callback`).trim();
    if (!clientId) return res.status(503).send('GitHub sign-in is not configured on the server.');

    const state = crypto.randomBytes(32).toString('hex');
    res.setHeader('Set-Cookie', `manlung_github_oauth_state=${encodeURIComponent(state)}; ${githubCookieOptions()}`);
    ...
    return res.status(500).send('Could not start GitHub sign-in.');
  }
});
...
router.get('/github/callback', async (req, res) => {
  ...
  try {
    ...
    const redirectUri = String(process.env.GITHUB_REDIRECT_URI || `${appUrl}/api/auth/github/callback`).trim();
    if (!clientId || !clientSecret) return res.status(503).send('GitHub sign-in is not configured on the server.');
    ...
    if (!githubToken) throw new Error(tokenResponse.data?.error_description || 'GitHub did not return an access token.');

    const headers = { Authorization: `Bearer ${githubToken}`, Accept: 'application/vnd.github+json', 'User-Agent': 'Manlung-Recovery' };
    const profile = (await axios.get('https://api.github.com/user', { headers, timeout: 10000 })).data;
    const emails = (await axios.get('https://api.github.com/user/emails', { headers, timeout: 10000 })).data;
    ...
    if (!email) return res.status(400).send('Your GitHub account does not have a verified email address available.');

    let client = await User.findByEmail(email);
    if (client && client.role !== 'client') {
      return res.redirect(`/login.html?oauth_error=${encodeURIComponent('This email belongs to an Admin/Owner account. Please use Admin Sign In.')}`);
    }

    if (!client) {
      const baseName = String(profile?.name || profile?.login || email.split('@')[0]).trim().slice(0, 200) || 'GitHub Client';
      let username = baseName;
      const existingUsername = await User.findByUsername(username);
      if (existingUsername) username = `${baseName}-${String(profile?.id || 'github').slice(0, 8)}`.slice(0, 80);
      client = await User.create({
        username,
        email,
        phone: null,
      ...
      sendEmail({
        to: client.email,
        subject: 'Welcome to Manlung Recovery — Account Created Successfully',
        html: `<p>Hello ${String(client.username).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')},</p><p>Your Manlung Recovery client account was created successfully using GitHub.</p><p><a href="${appUrl}">Open Manlung Recovery</a></p>`
      }).catch(error => console.error('GitHub welcome email failed:', error?.message || error));
    }

    const token = signToken(client);
    const userJson = encodeURIComponent(JSON.stringify({
      id: client.id, email: client.email, username: client.username, role: client.role
    }));
