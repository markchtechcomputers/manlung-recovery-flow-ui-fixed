...
**Evidence upload policy (hardened)**: the application accepts **PDF, JPG/JPEG, and PNG only**.
PDF files are limited to 10 MB and JPG/JPEG/PNG files to 5 MB each, with a maximum of
10 files per case submission. ZIP, Office documents, video, GIF and every other file
type are rejected by both the browser UI and the server. The server also validates MIME
type, filename extension, and file signature (magic bytes) before anything reaches
Supabase Storage; the browser `accept` attribute is only a usability layer and is not
