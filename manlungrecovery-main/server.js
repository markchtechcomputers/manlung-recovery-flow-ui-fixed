  ...
  helmet({
    contentSecurityPolicy: {
      ...
      directives: {
        ...
        connectSrc: ["'self'", 'https://*.supabase.co', 'https://api.paystack.co', 'wss:'],
...
// Defense in depth for the HttpOnly admin session cookie. SameSite=Strict
// blocks normal cross-site browser requests; this Origin check also rejects
// forged state-changing requests when a browser supplies the admin cookie.
// Bearer-token API clients and third-party webhooks are not affected.
const COOKIE_AUTH_NAME = 'manlung_admin_session';
const STATE_CHANGING_METHODS = new Set(['POST', 'PUT', 'PATCH', 'DELETE']);

app.use('/api/', (req, res, next) => {
  if (!STATE_CHANGING_METHODS.has(req.method)) return next();

  const hasAdminCookie = String(req.headers.cookie || '')
    .split(';')
    .some(part => part.trim().startsWith(`${COOKIE_AUTH_NAME}=`));
...
app.use('/api/auth/admin/mfa/login', loginLimiter);
...
function getClientIp(req) {
  // Express is configured with trust proxy=1 for the deployment proxy. Use req.ip
  // rather than accepting an arbitrary X-Forwarded-For header supplied by a caller.
...
const scannerLimiter = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  // Let express-rate-limit use Express's validated client IP handling.
  // Do not supply a custom keyGenerator that can return an invalid value.
  message: {
    success: false,
...
function isPrivateIPv6(ip) {
  ...
  // RFC 1918-style IPv6/private, link-local, multicast, unspecified and documentation ranges.
  return (
    value === '::' || value === '::1' ||
...
async function resolvePublicAddresses(hostname) {
  ...
  // DNS results must contain a real IPv4/IPv6 address before they can
  // be used for the outbound request. Some runtimes/providers can return
  // incomplete records, so never pass an undefined address to Node's
  // HTTP lookup callback.
  ...
  if (!validAddresses.length) {
    throw new Error('No valid public DNS address found.');
  }

  const privateAddress = validAddresses.find(entry => isPrivateAddress(entry.address));

  if (privateAddress) {
    throw new Error('The destination resolves to a private or reserved network address.');
  }

  return validAddresses.filter(entry => !isPrivateAddress(entry.address));
}

async function requestWebsite(targetUrl, addresses, timeoutMs = 10000) {
  const validated = Array.isArray(addresses)
    ? addresses.filter(entry =>
        entry &&
        typeof entry.address === 'string' &&
        net.isIP(entry.address) === Number(entry.family)
      )
    : [];

  if (!validated.length) {
    throw new Error('No validated public address available.');
  }

  let lastError = null;

  // Some legitimate sites publish several A/AAAA records and one path may be
  // temporarily unreachable. Try each already-validated public address without
  // performing a second DNS lookup.
  for (const selected of validated) {
    try {
      return await new Promise((resolve, reject) => {
        ...
        const request = client.request(
          parsed,
          {
            ...
            timeout: timeoutMs,
            headers: {
              'User-Agent': 'Manlung-Recovery-Safety-Scanner/1.2',
              'Accept': 'text/html,application/xhtml+xml,application/json;q=0.9,*/*;q=0.8'
            },
            // Pin the outbound connection to the DNS result we already validated.
            lookup: (_hostname, options, callback) => {
              // Node may request all DNS addresses. Return the correct
              // callback shape in both modes so the selected validated
              // address is never interpreted as undefined.
...
function normalizeUrl(value) {
  ...
  if (net.isIP(host) && isPrivateAddress(host)) {
    throw new Error('Private or reserved IP addresses cannot be scanned.');
  }
...
function analyzeDomain(hostname) {
  ...
  if (hostname.split('.').some(label => label.startsWith('xn--'))) {
    warnings.push('The hostname contains an internationalized/punycode label. Verify the exact spelling before trusting it.');
  }
...
app.post('/api/link-scanner/scan', scannerLimiter, async (req, res) => {
  ...
  try {
    ...
    if (finalParsed.protocol === 'https:' && finalResponse.tls?.authorized === false) {
      checks.https = {
        status: 'critical',
        label: 'HTTPS / TLS',
        details: 'HTTPS is available, but the server certificate could not be independently trusted by the scanner.'
      };
    }
    ...
    res.json({
      ...
      scannedProtocol: finalParsed.protocol,
      statusCode: finalResponse.statusCode,
      contentType: finalResponse.headers['content-type'] || null,
      tls: finalResponse.tls || null,
      truncated: Boolean(finalResponse.truncated),
    ...
    const safeError = process.env.NODE_ENV === 'production'
      ? 'Unable to scan this website. Please check the URL and try again.'
      : (error.message || 'Unable to scan this website.');
