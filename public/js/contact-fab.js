...
(() => {
  ...
  function addLoadingIndicator() {
    ...
    style.textContent = `
      #manlung-page-loading{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:2147483646;width:104px;min-height:104px;padding:14px;border-radius:24px;display:grid;place-items:center;background:#fff;box-shadow:0 12px 40px rgba(0,0,0,.18);opacity:1;transition:opacity .35s ease}
      ...
      #manlung-page-loading:before{content:"•••";position:absolute;bottom:6px;letter-spacing:5px;font-size:17px;color:#2563eb;animation:manlungLoadingDots 1.1s infinite ease-in-out}
      @keyframes manlungLoadingDots{0%,100%{opacity:.3;transform:translateY(0)}50%{opacity:1;transform:translateY(-2px)}}
