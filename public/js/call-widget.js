...
(function injectCallWidgetStyles() {
  ...
  style.textContent = `
    #callWidget { position:fixed !important; z-index:2147483000 !important; pointer-events:auto !important; --call-green:#20c77a; --call-green-dark:#128a55; --call-red:#ef4444; --call-blue:#4f8cff; --call-bg:#0b1428; --call-card:#111d35; --call-border:rgba(255,255,255,.10); --call-muted:#9fb0ca; }
    #callWidgetBtn { position:relative !important; z-index:2147483001 !important; pointer-events:auto !important; touch-action:manipulation !important; min-height:48px !important; padding:.72rem 1.05rem !important; border-radius:999px !important; background:linear-gradient(135deg,#20c77a,#128a55) !important; box-shadow:0 10px 30px rgba(18,138,85,.30) !important; transition:transform .2s ease,box-shadow .2s ease !important; }
    ...
    .manlung-floating-call-icon svg{width:20px;height:20px;display:block;color:#fff}
    @media(max-width:600px){.manlung-floating-call-icon svg{width:24px;height:24px}}
...
(function(){
  let currentPeer=null,currentSessionId=null,pollTimer=null,currentCallbackId=null,callbackPollTimer=null;
  ...
  function buildWidget(){const existing=document.getElementById('callWidget');if(existing)return existing;const wrap=el(`<div id="callWidget" style="position:fixed;bottom:calc(12px + env(safe-area-inset-bottom));right:12px;z-index:200;font-family:'Inter',-apple-system,sans-serif;"><button id="callWidgetBtn" type="button" aria-haspopup="dialog" aria-expanded="false" aria-controls="manlungContactMenu" title="Contact Manlung Recovery" style="background:#1f6e4a;color:#fff;border:none;border-radius:50px;padding:0.7rem 1.1rem;font-weight:600;font-size:0.85rem;cursor:pointer;box-shadow:0 6px 20px rgba(0,0,0,0.25);display:flex;align-items:center;gap:8px;"><span class="manlung-floating-call-icon" aria-hidden="true"><svg viewBox="0 0 24 24" role="img" aria-label="Phone"><path d="M6.6 2.8 9.2 2.2c.7-.2 1.4.2 1.7.9l1.2 2.9c.2.6.1 1.2-.4 1.6L10 9.2a15.8 15.8 0 0 0 4.8 4.8l1.6-1.7c.4-.5 1-.6 1.6-.4l2.9 1.2c.7.3 1.1 1 .9 1.7l-.6 2.6c-.2.8-.9 1.4-1.7 1.4C10.6 18.8 5.2 13.4 5.2 6.5c0-.8.6-1.5 1.4-1.7Z" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span id="callWidgetLabel">Contact Us</span></button><div id="manlungContactMenu" role="dialog" aria-modal="false" aria-labelledby="manlungContactTitle" hidden><div class="manlung-contact-menu-header"><div><strong id="manlungContactTitle">Contact Manlung Recovery</strong><span>Choose how you would like to reach us.</span></div><button id="manlungContactClose" type="button" aria-label="Close contact options" title="Close"><i class="fas fa-times" aria-hidden="true"></i></button></div><div class="manlung-contact-menu-actions"><button class="manlung-contact-choice manlung-contact-call" type="button" data-contact-action="call" aria-label="Call Admin" title="Call Admin"><span class="manlung-contact-choice-icon" aria-hidden="true"><img src="https://i.postimg.cc/CxLDpd5z/istockphoto-971654072-612x612-Photoroom.png" alt=""></span><span class="manlung-contact-choice-label">Call Admin</span></button><a class="manlung-contact-choice" data-contact-action="whatsapp" aria-label="WhatsApp" title="WhatsApp" href="https://wa.me/254745682493?text=Hello%20Manlung%20Recovery%20%F0%9F%91%8B%2C%20I%20need%20help%20with%20my%20recovery%20request." target="_blank" rel="noopener noreferrer"><span class="manlung-contact-choice-icon" aria-hidden="true"><img src="https://i.postimg.cc/ZR3vxdsW/whatsapp-whatsapp-app-logo-in-green-circle-AZU1A2SU-t-Photoroom.png" alt=""></span><span class="manlung-contact-choice-label">WhatsApp</span></a><a class="manlung-contact-choice" data-contact-action="email" aria-label="Email" title="Email" href="mailto:manlungrecovery@outlook.com?subject=Manlung%20Recovery%20Support"><span class="manlung-contact-choice-icon" aria-hidden="true"><img src="https://i.postimg.cc/vHXghMhL/125376099-mail-symbol-icon-red-simple-with-rounded-corners-isolated-vector-illustration-Photoroom.png" alt=""></span><span class="manlung-contact-choice-label">Email</span></a><a class="manlung-contact-choice" data-contact-action="telegram" aria-label="Telegram" title="Telegram" href="https://t.me/manlungtechcitybot" target="_blank" rel="noopener noreferrer"><span class="manlung-contact-choice-icon" aria-hidden="true"><img src="https://i.postimg.cc/MH6ZfMht/telegram-icon.jpg" alt=""></span><span class="manlung-contact-choice-label">Telegram</span></a></div></div><div id="callWidgetPanel" style="display:none;position:absolute;bottom:56px;right:0;width:min(320px,calc(100vw - 24px));max-height:calc(100vh - 100px);overflow:auto;background:#101f42;color:#e6ecf5;border:1px solid #29385a;border-radius:16px;padding:1.1rem;box-shadow:0 10px 30px rgba(0,0,0,0.35);"><div style="display:flex;justify-content:flex-end;margin:-0.35rem -0.35rem 0.25rem 0;"><button id="callWidgetCloseBtn" type="button" aria-label="Close Call Admin panel" title="Close" style="width:34px;height:34px;border:0;border-radius:50%;background:#29385a;color:#fff;cursor:pointer;font-size:1rem;"><i class="fas fa-times"></i></button></div><div id="callWidgetContent"></div></div></div>`);document.body.appendChild(wrap);const button=document.getElementById('callWidgetBtn'),menu=document.getElementById('manlungContactMenu'),menuClose=document.getElementById('manlungContactClose'),callChoice=menu?.querySelector('[data-contact-action="call"]');button.addEventListener('click',toggleContactMenu);menuClose?.addEventListener('click',()=>closeContactMenu({restoreFocus:true}));callChoice?.addEventListener('click',()=>{closeContactMenu();openCallAdminPanel();});document.addEventListener('keydown',handleContactKeydown);document.addEventListener('click',handleContactOutsideClick);return wrap;}
