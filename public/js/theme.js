(() => {
  ...
  function installHeaderPolish(){if(document.getElementById('manlung-header-icon-polish'))return;const style=document.createElement('style');style.id='manlung-header-icon-polish';style.textContent=`
    ...
    .site-header .header-actions .btn:hover i{
      transform:none!important;
    }
    ...
    @media(max-width:700px){
      .site-header .header-actions a i,
      .site-header .header-actions button i,
      .site-header .header-actions .btn i{font-size:1rem!important}
      .site-header .header-actions button[data-theme-toggle="true"]{
        position:static!important;
        width:auto!important;
        height:auto!important;
    ...
    @media(max-width:480px){
      ...
      .site-header .header-actions button[data-theme-toggle="true"] i{font-size:1rem!important}
    }
  `;document.head.appendChild(style)}
  ...
  function boot(){convertLegacyButtons();bindThemeButtons();createThemeButton();bindThemeButtons();setTheme(readTheme(),false);installCallIconFix();installHeaderPolish();loadAdminCallIcons();loadContactFab();loadManlungAI();loadClientCaseChat();installHeroVideoPlaylist();window.setManlungTheme=theme=>setTheme(theme,true);window.toggleDarkMode=()=>setTheme(readTheme()===DARK?LIGHT:DARK,true)}
