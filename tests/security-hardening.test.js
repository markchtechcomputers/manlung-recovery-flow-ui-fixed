...
test('login lockout is capped at three failures', () => {
  ...
  assert.match(userModel, /MAX_LOGIN_FAILURES\s*=\s*3/);
  assert.match(userModel, /failed_login_attempts/);
  assert.match(userModel, /login_locked_until/);
  assert.match(userModel, /failures\s*>=\s*MAX_LOGIN_FAILURES/);
});
...
test('security logging avoids raw login identifiers', () => {
  const security = fs.readFileSync(path.join(__dirname, '..', 'middleware', 'inputSecurity.js'), 'utf8');
  assert.match(security, /securityLog\(/);
  assert.match(security, /ipHash/);
  assert.doesNotMatch(security, /loginIdentifier\s*[:=]/);
...
test('role-based session expiry and revoked-session checks are present', () => {
  ...
  assert.match(auth, /24\s*\*\s*60\s*\*\s*60/);
  assert.match(auth, /7\s*\*\s*24\s*\*\s*60\s*\*\s*60/);
...
test('admin sessions are issued for 24h and clients for 7d', () => {
  const auth = fs.readFileSync(path.join(__dirname, '..', 'routes', 'auth.js'), 'utf8');
  assert.match(auth, /admin:\s*'24h'/);
  assert.match(auth, /owner:\s*'24h'/);
  assert.match(auth, /client:\s*'7d'/);
  assert.match(auth, /maxAge:\s*SESSION_MAX_AGE_MS\.admin/);
});

test('donation total exposes a configurable target and percentage', () => {
